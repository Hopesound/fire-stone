...
급경사지 현황은 `tools/convert-steep-slopes.mjs`로 `src/data/steep-slope-sites.generated.js`에 변환되며, 지도에는 갈색 점으로 표시됩니다. 원본 CSV에 위경도가 없으므로 현재 지도 위치는 주소의 시군 중심과 주소별 오프셋을 이용한 추정 위치입니다.
...
원본 급경사지 CSV에는 위경도 컬럼이 없으므로 초기 지도 표시는 시군 중심 기반 추정 위치입니다. 실제 주소 위치에 가깝게 보려면 화면에서 V-World 인증키와 도메인을 입력한 뒤 `급경사지 주소좌표 보정`을 실행합니다.

- V-World 주소 API `request=getCoord`로 급경사지 지번주소를 `EPSG:4326` 좌표로 변환합니다.
- 변환된 좌표는 브라우저 `localStorage`에 저장되어 다음 접속 때도 우선 사용됩니다.
- 보정 전 급경사지는 갈색, 주소좌표 보정 후 급경사지는 파란색으로 표시됩니다.
